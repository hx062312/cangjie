...
def main(args):
    ...
    if not os.path.exists(f"data/java/type_resolution/{project}/type_description.json"):
        ...
        types = {}
        with open(f"data/java/type_resolution/{project}/s1_input.json") as f:
            types = json.load(f)
        ...
        for type_ in types:
            ...
            type_descriptions[type_] = {"link": "", "text": "", "summarized_text": ""}

        with open(
            f"data/java/type_resolution/{project}/type_description.json", "w"
        ) as f:
            json.dump(type_descriptions, f, indent=4)

    type_descriptions = {}
    with open(f"data/java/type_resolution/{project}/type_description.json") as f:
    ...
    with open(f"data/java/type_resolution/{project}/type_description.json", "w") as f:
        json.dump(type_descriptions, f, indent=4)
